module bmult_mod
    ...
    contains
        ...
        subroutine left_Bmult(S, A, l, sigma)
            ...
            ! Updates:
            !
            ! Todo: adapt to be like this! A = B(l) * A
            !
            ! Where B(l) is the lth single particle propagator from imaginary
            ! time (l-1)*dtau to l*dtau with spin sigma (1 for up, -1 for dn)
            !
        ...
        subroutine right_Bmultexact(S, A, l, sigma)
            !
            ! Updates:
            !
            ! A = A * B(l)
        ...
        subroutine left_Bmultexact(S, A, l, sigma)
            !
            ! Updates:
            !
            ! A = B(l) * A
        ...
        endsubroutine left_Bmultexact


        subroutine right_Binvmultexact(S, A, l, sigma)
            !
            ! Updates:
            !
            ! A = A * inv(B(l))
        ...
        endsubroutine right_Binvmultexact


        subroutine left_Binvmultexact(S, A, l, sigma)
            !
            ! Updates:
            !
            ! A = inv(B(l)) * A
        ...
        endsubroutine left_Binvmultexact


        subroutine make_Bexact(S, A, l, sigma)
            !
            ! Sets:
            !
            ! A = B(l)
            ...
            call dlaset('A', S%N, S%N, 0.0_dp, 1.0_dp, A, S%N)
            ! A = A * B(l)
            call right_Bmultexact(S, A, l, sigma)


        endsubroutine make_Bexact
