program main
    ...
    call read_matrix("ex33.mtx", A, m, n)
    allocate(P(n, n))
    allocate(Q(n, n))
    ...
    write(terminal, "(a)") "qrcp results ---------------------"
    ! call print_matrix(A, terminal, "A = ")
    ! call print_matrix(P, terminal, "P = ")
    ! call print_matrix(Q, terminal, "Q = ")
    ! call print_matrix(R, terminal, "R = ")
    ! call print_matrix(matmul(A, P), terminal, "AP = ")
    ! call print_matrix(matmul(Q, R), terminal, "QR = ")
    ! call print_matrix(matmul(A, P) - matmul(Q, R), terminal, "AP - QR = ")
    write(terminal, "(a)") "----------------------------------"
    ...
    write(terminal, "(a)") "ssrqrcp results ------------------"
    write(terminal, "(a)") "parameters:"
    write(terminal, "(a, i4)") "k  = ", k
    write(terminal, "(a, i4)") "pp = ", pp
    ! call print_matrix(A, terminal, "A = ")
    ! call print_matrix(P, terminal, "P = ")
    ! call print_matrix(Q(:, :k), terminal, "Q(:, :k) = ")
    ! call print_matrix(R, terminal, "R = ")
    ! call print_matrix(matmul(A, P), terminal, "AP = ")
    ! call print_matrix(matmul(Q(:, :k), R), terminal, "Q(:, :k)R = ")
    ! call print_matrix(matmul(A, P) - matmul(Q(:, :k), R), terminal, "AP - Q(:, :k)R = ")
    write(terminal, "(a)") "----------------------------------"
    write(terminal, "(a, f17.8)") "frobenius-norm relative error of approximation = ", dmnorm(matmul(A, P) - matmul(Q(:, :k), R), "f") / dmnorm(matmul(A, P), "f")
    ...
    contains
        ...
        subroutine ssrqrcp(A, k, pp, P, Q, R)
            ...
            real(dp) :: R11(k, k), R12(k, size(A, 2)-k)
        ...
        subroutine qr(A, Q, R)
            real(dp), intent(in)  :: A(:, :)
            real(dp), intent(out) :: Q(size(A, 1), size(A, 1))
            real(dp), intent(out) :: R(size(A, 2), size(A, 2))
        ...
        subroutine qrcp(A, P, Q, R)
            ...
            real(dp), intent(out) :: Q(size(A, 1), size(A, 1))
            real(dp), intent(out) :: R(min(size(A, 1), size(A, 2)), min(size(A, 1), size(A, 2)))
        ...
        function dgeq_to_r(A) result(R)
            ...
            real(dp) :: R(min(size(A, 1), size(A, 2)), min(size(A, 1), size(A, 2)))
            integer  :: i, j
    ...
    subroutine read_matrix(filename, A, m, n)
        implicit none
        ...
        ! Skip header and comments starting with %
        do
        ...
        ! Read dimensions and number of non-zero entries
        read(line, *) m, n, L
    
        ! Allocate and initialize the dense matrix
        allocate(A(m, n))
        A = 0.0d0
